...
print_daily_breakdown() {
    ...
    data="$(daily_breakdown_in_window "$start_epoch" "$end_epoch")"

    if [[ -z "$data" ]]; then
        echo "No daily activity found."
        return
    fi

    printf '%-12s %-9s %-8s %-7s %-7s %-7s\n' \
        "Date" "Sessions" "Commands" "Errors" "Notes" "Total"
    print_divider

    while IFS='|' read -r day sessions commands errors notes total; do
        printf '%-12s %-9s %-8s %-7s %-7s %-7s\n' \
            "$day" "$sessions" "$commands" "$errors" "$notes" "$total"
    done <<< "$data"
}
...
print_top_session() {
    ...
    local session_id mode context_type context_value end_time commands errors notes total noise productive
    IFS='|' read -r session_id mode context_type context_value end_time commands errors notes total noise productive <<< "$line"
...
print_recent_excerpts() {
    ...
    data="$(recent_excerpts_in_window "$start_epoch" "$end_epoch" 8)"
...
main() {
    ...
    echo "Sapphire Sentinel Monthly Report"
    print_divider
    printf 'Window:        Last 30 days\n'
    printf 'Sessions:      %s\n' "$sessions_count"
    printf 'Commands:      %s\n' "$commands"
    printf 'Errors:        %s\n' "$errors"
    printf 'Notes:         %s\n' "$notes"
    ...
    print_daily_breakdown "$start_epoch" "$end_epoch"
